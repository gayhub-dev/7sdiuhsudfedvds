...
public class ProtocolFactoryImpl implements ProtocolFactory {
    ...
    public ReceivingAsync createReceivingAsync(IncomingDatagramMessage incomingDatagramMessage) throws ProtocolCreationException {
        ...
        if (incomingDatagramMessage.getOperation() instanceof UpnpRequest) {
            int i7 = C16721.$SwitchMap$org$fourthline$cling$model$message$UpnpRequest$Method[((UpnpRequest) incomingDatagramMessage.getOperation()).getMethod().ordinal()];
            if (i7 == 1) {
    ...
    public ReceivingSync createReceivingSync(StreamRequestMessage streamRequestMessage) throws ProtocolCreationException {
        ...
        if (getUpnpService().getConfiguration().getNamespace().isControlPath(streamRequestMessage.getUri())) {
            ...
            StringBuilder sbM112a = C0413b.m112a("Fixing trailing garbage in event message path: ");
            sbM112a.append(streamRequestMessage.getUri().getPath());
    ...
    public SendingSearch createSendingSearch(UpnpHeader upnpHeader, int i7) {
        return new SendingSearch(getUpnpService(), upnpHeader, i7);
    }
    ...
    public SendingSubscribe createSendingSubscribe(RemoteGENASubscription remoteGENASubscription) throws ProtocolCreationException {
        try {
            ...
            throw new ProtocolCreationException("Failed to obtain local stream servers (for event callback URL creation) from router", e7);
        }
    }
    ...
    public ProtocolFactoryImpl(UpnpService upnpService) {
        Logger logger = log;
        StringBuilder sbM112a = C0413b.m112a("Creating ProtocolFactory: ");
