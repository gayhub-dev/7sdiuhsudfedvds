...
public final class HttpHeaders {
    ...
    */
    ...
    public static List<Challenge> parseChallenges(Headers headers, String str) {
        ...
        for (int i7 = 0; i7 < headers.size(); i7++) {
            if (str.equalsIgnoreCase(headers.name(i7))) {
                parseChallengeHeader(arrayList, new Buffer().writeUtf8(headers.value(i7)));
            }
        }
        return arrayList;
    }

    public static int parseSeconds(String str, int i7) throws NumberFormatException {
        try {
    ...
    private static String repeat(char c7, int i7) {
        char[] cArr = new char[i7];
        Arrays.fill(cArr, c7);
        return new String(cArr);
    }

    private static int skipAll(Buffer buffer, byte b7) {
        int i7 = 0;
