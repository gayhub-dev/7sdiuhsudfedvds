...
public final class RetryAndFollowUpInterceptor implements Interceptor {
    ...
    private Request followUpRequest(Response response, Route route) throws ProtocolException {
        ...
        if (!sameConnection(response, httpUrlResolve)) {
            builderNewBuilder.removeHeader(com.alibaba.sdk.android.oss.common.utils.HttpHeaders.AUTHORIZATION);
        }
    ...
    private boolean recover(IOException iOException, StreamAllocation streamAllocation, boolean z6, Request request) throws IOException {
        streamAllocation.streamFailed(iOException);
        if (this.client.retryOnConnectionFailure()) {
            return !(z6 && (request.body() instanceof UnrepeatableRequestBody)) && isRecoverable(iOException, z6) && streamAllocation.hasMoreRoutes();
        }
        return false;
    }

    private int retryAfter(Response response, int i7) {
        String strHeader = response.header("Retry-After");
        return strHeader == null ? i7 : strHeader.matches("\\d+") ? Integer.valueOf(strHeader).intValue() : ActivityChooserView.ActivityChooserViewAdapter.MAX_ACTIVITY_COUNT_UNLIMITED;
    }
        ...
        while (!this.canceled) {
            try {
                try {
                    ...
                    if (!recover(e9, streamAllocation, !(e9 instanceof ConnectionShutdownException), request)) {
                        throw e9;
                    }
                }
                ...
                if (i8 > 20) {
                    streamAllocation.release();
                    throw new ProtocolException(C0079a.m93a("Too many follow-up requests: ", i8));
                }
